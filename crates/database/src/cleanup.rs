...
/// Map from graph name to time of session start
...
impl UserSessionExpiries {
    ...
    pub fn insert(&mut self, graph: &str) {
...
async fn run_cleanup(
    ...
    loop {
        ...
        let to_remove = {
            ...
            let session_duration = VOWLGRAPHER_ENVIRONMENT.session_duration;

            let to_remove: HashSet<_> = ages
                .0
                .iter()
                .filter(|(_, start_time)| remove_start_time > **start_time + session_duration)
        ...
        info!("cleaning {} graphs", to_remove.len());
