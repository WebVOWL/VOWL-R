...
impl GraphDisplayDataSolutionSerializer {
    ...
    #[expect(
        ...
        reason = "this is single-threaded context"
    )]
        ...
        match Self::resolve_so(data_buffer, &triple)? {
            (Some(subject_term_id), Some(object_term_id)) => {
                ...
                data_buffer
                    .edge_label_buffer
                    .write()?
                    .insert(edge.clone(), label.unwrap_or_else(|| new_type.to_string()));
                return Ok(Some(edge));
            }
            (None, Some(_)) => {
                debug!(
    ...
    #[expect(
        clippy::significant_drop_tightening,
        reason = "drop is at the end of the last if-block which makes little sense"
    )]
    fn merge_restriction_state(
        ...
        let maybe_old_state = { data_buffer.restriction_buffer.write()?.remove(&old_term_id) };
        let Some(old_state) = maybe_old_state else {
            return Ok(());
        };
    ...
    fn insert_inverse_of(
        ...
        let (left_subject_raw, left_object_raw) =
            if let Some(endpoints) = self.inverse_edge_endpoints(data_buffer, &left_property)? {
                endpoints
            } else {
        ...
        let (right_subject_raw, right_object_raw) =
            if let Some(endpoints) = self.inverse_edge_endpoints(data_buffer, &right_property)? {
                endpoints
            } else {
    ...
    #[expect(
        clippy::significant_drop_tightening,
        reason = "drop is at the end of the statement which makes little sense"
    )]
    ...
    fn create_named_node(iri: &String) -> Result<NamedNode, SerializationError> {
    ...
    fn create_blank_node(id: &String) -> Result<BlankNode, SerializationError> {
        Ok(BlankNode::new(id)
    ...
    fn create_term(term: &String) -> Result<Term, SerializationError> {
                ...
                match uri.as_ref() {
                    ...
                    owl::COMPLEMENT_OF => {
                        ...
                        if triple.object_term_id.is_some()
                            && let Some(index) = Self::resolve(data_buffer, triple.subject_term_id)?
    ...
    fn insert_characteristic(
        ...
        if is_reserved(&resolved_iri) {
            debug!(
                "Skipping characteristic '{characteristic}' for reserved built-in '{resolved_iri}'"
            );
            return Ok(SerializationStatus::Serialized);
        }
